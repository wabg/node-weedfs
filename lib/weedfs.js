        ...
        return new Promise(function (resolve, reject) {
            var req = http.request(url.parse(self.baseURL + "dir/assign?" + qs.stringify(opts)), function (res) {
                ...
                res.on("end", function () {
                    ...
                    if (json && json.url && self.DataCenterServer) {
                        json.url = json.url.replace(/.*\:/, `${self.DataCenterServer}:`);
                    }
                    if (json && json.publicUrl && self.DataCenterServer) {
                        json.publicUrl = json.publicUrl.replace(/.*\:/, `${self.DataCenterServer}:`);
                    }
                    return resolve(json);
                });
            });
