        ...
        return new Promise(function (resolve, reject) {
            var req = http.request(url.parse(self.baseURL + "dir/assign?" + qs.stringify(opts)), function (res) {
                ...
                res.on("end", function () {
                    ...
                    if (json && json.url && self.clientOpts.DataCenterServer) {
                        json.url = json.url.replace(/.*\:/, `${self.clientOpts.DataCenterServer}:`);
                    }
                    if (json && json.publicUrl && self.clientOpts.DataCenterServer) {
                        json.publicUrl = json.publicUrl.replace(/.*\:/, `${self.clientOpts.DataCenterServer}:`);
                    }
                    return resolve(json);
                });
            });
