...
function WeedFSClient(opts) {
    ...
    if (!this.clientOpts.protocol) this.clientOpts.protocol = 'http';
    this.baseURL = this.clientOpts.protocol + "://" + this.clientOpts.server + ":" + this.clientOpts.port + "/";
}

function handingJSON(json, host) {
    if (json && json.url && host) {
        json.url = json.url.replace(/.*\:/, `${host}:`);
    }
    if (json && json.publicUrl && host) {
        json.publicUrl = json.publicUrl.replace(/.*\:/, `${host}:`);
    }
    return json
}
...
WeedFSClient.prototype = {
    setDataCenterServer: function (json) {
        const host = this.clientOpts.DataCenterServer;
        // If the owner returned in docker is not what you want.
        // Can be set by DataCenterServer
        json = handingJSON(json, host);
        if (json && json.locations && json.locations.length > 0) {
            json.locations = json.locations.map(function (item) {
                item = handingJSON(item, host);
                return item;
            });
        }
        return json;
        ...
        return new Promise(function (resolve, reject) {
            var req = http.request(url.parse(self.baseURL + "dir/assign?" + qs.stringify(opts)), function (res) {
                ...
                res.on("end", function () {
                    ...
                    // If the owner returned in docker is not what you want.
                    // Can be set by DataCenterServer
                    json = self.setDataCenterServer(json)
                    return resolve(json);
                });
            });
        ...
        return self._assign(assignOpts).then(function (finfo) {
            ...
            for (var i = 0; i < opts.count; i++) {
                proms.push(new Promise(function (resolve, reject) {
                    ...
                    var urlParts = url.parse(self.clientOpts.protocol + "://" + (self.usePublicUrl ? finfo.publicUrl : finfo.url) + "/" + finfo.fid + (opts.count == 1 ? "" : "_" + i));
    ...
    volumeStatus: function (host) {
        return new Promise(function (resolve, reject) {
            var req = http.request(url.parse(this.clientOpts.protocol + "://" + host + "/status"), function (res) {
                let body = "";
                let err;
        ...
        return self.find(fid, opts).then(function (res) {
            return new Promise(function (resolve, reject) {
                if (res.locations.length) {
                    let options = Object.assign({}, url.parse(self.clientOpts.protocol + "://" + (self.usePublicUrl ? res.locations[0].publicUrl : res.locations[0].url) + "/" + fid));
                    if (opts && opts.headers) {
                        options.headers = opts.headers;
                    }
                    if (opts && opts.query) {
                        const query = new URLSearchParams(opts.query);
                        options.path = `${options.path}?${query.toString()}`;
                        options.href = `${options.href}?${query.toString()}`;
                    }
                    let req = http.request(options, function (res) {
        ...
        return self.find(fid, opts).then(function (result) {
            return new Promise(function (resolve, reject) {
                ...
                for (var i = 0, len = result.locations.length; i < len; i++) {
                    proms.push(new Promise(function (resolve, reject) {
                        var req = http.request(Object.assign(url.parse(self.clientOpts.protocol + "://" + (self.usePublicUrl ? result.locations[i].publicUrl : result.locations[i].url) + "/" + fid), {
                            "method": "DELETE"
                        }), function (res) {
                            if (res.statusCode === 404) {
                                var err = new SeaweedFSError("file '" + fid + "' not found");
                            ...
                            res.on("end", function () {
                                var buffer = Buffer.concat(tmp);
                                var payload = JSONParse(buffer.toString("utf-8"));
    ...
    vacuum: function (opts) {
        ...
        return new Promise(function (resolve, reject) {
            var req = http.request(url.parse(self.baseURL + "vol/vacuum?" + qs.stringify(opts)), function (res) {
                ...
                res.on("end", function () {
                    var buffer = Buffer.concat(tmp);
                    resolve(JSONParse(buffer.toString("utf8")));
                });
            });
            req.on("error", function (err) {
                reject(err);
