        ...
        return self._assign(assignOpts).then(function (finfo) {
            ...
            for (var i = 0; i < opts.count; i++) {
                proms.push(new Promise(function (resolve, reject) {
                    ...
                    var urlParts = url.parse(this.clientOpts.protocol + "://" + (self.usePublicUrl ? finfo.publicUrl : finfo.url) + "/" + finfo.fid + (opts.count == 1 ? "" : "_" + i));
        ...
        return self.find(fid, opts).then(function (res) {
            return new Promise(function (resolve, reject) {
                if (res.locations.length) {
                    let options = Object.assign({}, url.parse(this.clientOpts.protocol + "://" + (self.usePublicUrl ? res.locations[0].publicUrl : res.locations[0].url) + "/" + fid));
        ...
        return self.find(fid, opts).then(function (result) {
            return new Promise(function (resolve, reject) {
                ...
                for (var i = 0, len = result.locations.length; i < len; i++) {
                    proms.push(new Promise(function (resolve, reject) {
                        var req = http.request(Object.assign(url.parse(this.clientOpts.protocol + "://" + (self.usePublicUrl ? result.locations[i].publicUrl : result.locations[i].url) + "/" + fid), {
                            "method": "DELETE"
                        }), function (res) {
                            if (res.statusCode === 404) {
                                var err = new SeaweedFSError("file '" + fid + "' not found");
